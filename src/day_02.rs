...
fn parse_input(input: &str) -> Vec<Report> {
    input
        .split('\n')
        .map(|line| line.split_whitespace().map(|val| val.parse().unwrap()).collect())
        .collect()
}
...
mod tests {
    ...
    use crate::day_02::{find_number_of_dampened_safe_reports, find_number_of_safe_reports, parse_input};
